...
import { Upload, Download, Settings, LayoutTemplate, Plus, Trash2 } from "lucide-react";
...
export function CollageCore() {
  ...
  return (
    <div className="flex h-[calc(100vh-3.5rem)] w-full flex-col md:flex-row bg-gray-50">
      {/* Left: Templates (20%) */}
      <div className="w-full md:w-[20%] border-r bg-white p-4 overflow-y-auto shrink-0">
            ...
            <div className="grid grid-cols-2 gap-2">
              {images.map((img, i) => (
                <div key={i} className="relative group aspect-square rounded-md overflow-hidden border">
                  ...
                  <img src={img} alt={`上传的图片 ${i + 1}`} className="w-full h-full object-cover" />
      ...
      <div className="flex w-full md:w-[55%] flex-col items-center justify-center p-8 overflow-auto shrink-0 relative bg-gray-100/50">
        {images.length > 0 ? (
          ...
          <div className="flex flex-col items-center justify-center w-full max-w-md aspect-square border-2 border-dashed border-gray-300 rounded-xl bg-white shadow-sm">
            ...
            <Button onClick={() => document.getElementById("collage-upload-center")?.click()}>
              选择图片
            </Button>
            <input
          ...
          <div className="space-y-6 bg-gray-50 p-4 rounded-lg border">
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                ...
                <span className="text-sm font-mono bg-white px-2 py-1 rounded border text-gray-600">{settings.gap}px</span>
              </div>
              <Slider
                value={[settings.gap]}
                min={0}
                max={50}
                step={1}
                onValueChange={(val) => setSettings({ ...settings, gap: Array.isArray(val) ? val[0] : (val as unknown as number[])[0] || val as unknown as number })}
                className="py-2"
              />
            </div>
            ...
            <div className="space-y-3">
              <div className="flex justify-between items-center">
                ...
                <span className="text-sm font-mono bg-white px-2 py-1 rounded border text-gray-600">{settings.borderRadius}px</span>
              </div>
              <Slider
                ...
                max={50}
                step={1}
                onValueChange={(val) => setSettings({ ...settings, borderRadius: Array.isArray(val) ? val[0] : (val as unknown as number[])[0] || val as unknown as number })}
                className="py-2"
              />
            </div>
